...
class TicTacToeGame:
    ...
    def make_move(self, position, specific=None):
        ...
        self.active_player = (
            self.PLAYER if self.active_player == self.COMPUTER else self.COMPUTER
        )
        return True
    ...
    def check_winner(self, to_check):
        return (
            (self.board[7] == self.board[8] == self.board[9] == to_check)
            or (self.board[4] == self.board[5] == self.board[6] == to_check)
            or (self.board[1] == self.board[2] == self.board[3] == to_check)
            or (self.board[7] == self.board[4] == self.board[1] == to_check)
            or (self.board[8] == self.board[5] == self.board[2] == to_check)
            or (self.board[9] == self.board[6] == self.board[3] == to_check)
            or (self.board[7] == self.board[5] == self.board[3] == to_check)
            or (self.board[1] == self.board[5] == self.board[9] == to_check)
        )

    def check_draw(self):
        return (
            (self.EMPTY not in self.board.values())
            and (not self.check_winner(self.PLAYER))
            and not self.check_winner(self.COMPUTER)
        )
