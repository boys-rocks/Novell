...
class AFK(commands.Cog):
    ...
    async def on_message2(self, message):
        ...
        for i in list1:
            try:
                ...
                word = str(i)
                embed = discord.Embed(
                    title="Member currently AFK:",
                    description=results[word],
    ...
    @commands.command(help="Set status to afk")
    ...
    async def afkcommand(self, ctx, reason):
        ...
        try:
            collection.update_one(
                {"_id": "afk"}, {"$set": {str(ctx.author.id): reason}}
            )
            embed = discord.Embed(
                title="AFK set:", description=reason, color=discord.Color.random()
            )
