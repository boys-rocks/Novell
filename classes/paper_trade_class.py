...
class account():
    ...
    def buy(self, symbol, amount):
        ...
        if self.balance >= cost_and_coin[0]:
            self.recentTrades.append({"side":"buy","symbol":cost_and_coin[1],"amount":amount,"value":cost_and_coin[0]})
            ...
            return str(amount)+ " "+cost_and_coin[1]+" purchased for "+str(cost_and_coin[0])
        else:
    ...
    def sell(self, symbol, amount):
        if self.coins[symbol]:
            if self.coins[symbol] >= amount:
                cost_and_coin = getCost(symbol,amount)
                self.recentTrades.append({"side":"sell","symbol":symbol,"amount":amount,"value":cost_and_coin[0]})
                self.updateBalance(cost_and_coin[0])
